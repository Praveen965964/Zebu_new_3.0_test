...
import {userid,usession} from "../mixins/apiConnectionPool.js"
...
export class BrokerApi {
    ...
    async closedPositions(){
        ...
        //         const requiredPosition = formatRequiredPosition(PositionData);
            this._positions.map((requiredPosition) => {
                if(requiredPosition.qty == 0){
    ...
    async excMessages(){
        ...
        const getBrokerMessage = () => {
            const request = `jData={"uid":"${userid}"}&jKey=${usession}`;
            const brokres = callApi("GetBrokerMsg", request);
            return Promise.resolve(brokres);
        }
        const messageData = await getMessageData();
        const brokermessageData = await getBrokerMessage();
        ...
        if(brokermessageData[0]){
        if (brokermessageData[0].stat == "Ok") {
            brokermessageData.map((MessageData)=>{
                const v={
                    'id':i,
                    'time':MessageData.norentm,
                    'msg':MessageData.dmsg,
                    'exch':MessageData.msgtyp,
                }
                chartUnderstoodExcMessageData.push(v);
                i=i+1;
            });
    ...
    async placeOrder(preOrder,confirmId) {
        ...
        let amocheck = false;
        const symInfo = await checkFreezeQty(preOrder)
        const handler = async (params)=>{
            ...
            const pCode = preOrder.customFields['productType'];
            amocheck = preOrder.customFields.amoCheck;
        ...
        if(confirmId=="reverse"){
            ...
            let prcType;
            let sym = preOrder.symbol.split(":")[1]
            sym = sym.includes("&")?sym.replace("&","%26"):sym;
            if(preOrder.duration){
            ...
            if(confirmId == "close"){
                ...
                else if(params.stopLoss && params.takeProfit){
                    if(params.type == 4){
                        return this._host.showNotification("Order Error","Cannot place Order with SL-LMT for Cover Order and Bracket Order",0);
                    }
                    orderData = {
                        "uid":userid,
                        "actid":actid,
                        ...
                        "prd":"B",
                        "prc": params.type == 3 || params.type == 2 ? "0" : String(params.limitPrice || params.stopPrice || params.seenPrice),
                        "qty": symbolData.exchange == "MCX" ? String(preOrder.qty * symbolData.lot_size) : String(preOrder.qty),
                        "ret": "DAY",
                        "symbol_id": symbolData.token,
                        "tsym": sym,
                        "trantype": getTransactionType(params.side || 1).toUpperCase(),
                        "trgprc": params.type == 4 || params.type == 3 ? String(params.stopPrice) : "0",
                        "bpprc":String(Number(params.takeProfit - params.seenPrice).toFixed(2)),
                        "blprc":String(Number(params.seenPrice - params.stopLoss).toFixed(2))
                    };
                }
                else if(params.stopLoss && params.takeProfit==undefined){
                    if(params.type == 4){
                        return this._host.showNotification("Order Error","Cannot place Order with SL-LMT for Cover Order and Bracket Order",0);
                    }
                    orderData = {
                ...
                else if(params.stopLoss == undefined && params.takeProfit){
                    return this._host.showNotification("Order Error","For Cover Order, Place Order with Only Stop Loss. For Bracket Order, Place Order with both Take Profit and Stop Loss",0);
                }
        ...
        if(amocheck == true){
            ...
            const order = {
                ...
                limitPrice: params.limitPrice || params.stopPrice || params.seenPrice,
                profit: 0,
                qty: params.qty,
                side: params.side || 1,
                status: 6,
                ...
                type: params.type || 2,
                execution: params.customFields ? params.customFields['productType'] : '',
        ...
        let response;
        if(symInfo.frzqty != 0){
        if(Number(preOrder.qty) < Number(symInfo.frzqty)){
        response = await handler(preOrder);
        if(response){
        if (response.stat == "Ok") {
...
//         else if((((1 * symInfo.frzqtyCheck) + (symInfo.multipleOffrzQty * Number(symInfo.frzqty))) % (1+symInfo.multipleOffrzQty)) == 0){
//         for(let i = 0; i < symInfo.multipleOffrzQty+1; i++){
//         preOrder.qty = symInfo.frzqty;
//         response = await handler(preOrder);
//         if(response){
...
//         }
            ...
            for(let i = 0; i < symInfo.multipleOffrzQty; i++){
                preOrder.qty = symInfo.frzqty;
                response = await handler(preOrder);
                if(response){
                if (response.stat == "Ok") {
                ...
                preOrder.qty = symInfo.frzqtyCheck;
                response = await handler(preOrder);
                if(response){
                if (response.stat == "Ok") {
                    currentOrder.id = response.norenordno;
    ...
    async modifyOrder(order) {
            ...
            const handler = async (params)=>{
                ...
                if (originalOrder) {
                    ...
                    modifiedOrder.execution = params.customFields ? params.customFields['productType'] : '';
                ...
                if(params.type == 4 || params.type == 3){
                    orderData.trgprc = String(params.stopPrice)
                }
                const request = `jData=${JSON.stringify(orderData)}&jKey=${usession}`;
                ...
                positionList.map((positionData)=>{
                    ...
                    if (hasPositionAlready) {
                        ...
                        this._subscribeData(requiredPosition, "POS" + requiredPosition.id,(last)=>{
                            ...
                            requiredPosition.profit = Number((requiredPosition.last - requiredPosition.avgPrice) * qty);
                            requiredPosition.pospnl = Number((requiredPosition.last - requiredPosition.avgPrice) * qty) + requiredPosition.totalPl;
    ...
    async closeAllPositions(){
        ...
        var params = {
            title: 'Alert',
            body: 'No Open Position to Square Off',
            callback: (res) => {
                if (res) {
                    logMessage(res)
                }
    ...
    async previewOrder(order){
        const handler = async (params,type)=>{
            // console.log("Seen Price", params.seenPrice, params.limitPrice,params.stopPrice)
        if (order.duration)
        ;this._host.activateBottomWidget();
        const symbolData = window._symbolInfoMap[order.symbol]
    if(order.customFields != undefined){
    const pCode = order.customFields['productType'];
    var mySubString = pCode.substring(
        pCode.indexOf("(") + 1, 
        pCode.lastIndexOf(")")
    );
    ...
    if(type == "brokerage"){
        ...
        return Promise.resolve(checkFreezeQty(order))
    }
    // console.log("Res for Brokerage",margin);
}
        const response = await handler(order,"brokerage");
        const mres = await handler(order,"margin");
        const symInfo = await handler(order,"security")

        var orderpreviewResult;
        if(order.customFields.amoCheck == true){
            orderpreviewResult = {
                sections:[
    ...
    if(symInfo.frzqty != 0){
    if(Number(order.qty) < Number(symInfo.frzqty) || Number(order.qty) < Number(symInfo.frzqty)){
        orderpreviewResult.sections[0].rows.push({title:"Order Placement",value:`1 * ${symInfo.frzqtyCheck} Qty`})
    }
    // else if((((1 * symInfo.frzqtyCheck) + (symInfo.multipleOffrzQty * Number(symInfo.frzqty))) % (1+symInfo.multipleOffrzQty)) == 0){
    //     orderpreviewResult.sections[0].rows.push({title:"Split Order",value:`${symInfo.multipleOffrzQty + 1} * ${Number(symInfo.frzqty)} Qty`})
    // }
    else{
        let msg = `Order will be Placed as the following: \n${symInfo.multipleOffrzQty} * ${Number(symInfo.frzqty)} Qty \n1 * ${symInfo.frzqtyCheck} Qty`
        var params = {
            title: 'Alert',
            body: msg,
            callback: (res) => {
        ...
        window.tvWidget.showNoticeDialog(params);
        orderpreviewResult.sections[0].rows.push({title:"Split Order",value:`${symInfo.multipleOffrzQty} * ${Number(symInfo.frzqty)} Qty`})
        orderpreviewResult.sections[0].rows.push({title:"Split Order",value:`1 * ${symInfo.frzqtyCheck} Qty`})
    }
}
    ...
    async cancelOrder(orderId) {
        const order = this._orderById[orderId];
            ...
            ;
            // console.log("order details", order.product, order.borelatedorderid)
            if((order.product == "BO" || order.product == "H" || order.product == "CO") && order.borelatedorderid){
                const ordercancel = ()=>{
                    const cancelrequest = {
                                "uid": userid,
                                "norenordno": order.borelatedorderid,
                                "prd":order.product=="BO" ? "B" : "H"
                    };
                    const request = `jData=${JSON.stringify(cancelrequest)}&jKey=${usession}`;
                    const response = callApi("ExitSNOOrder",request);
                    return Promise.resolve(response);
            }
            ;
    ...
    async isTradable(symbol){
        if(symbol != undefined){
        const syminfo = await window._symbolInfoMap[symbol]
    ...
    let orderData={
        "uid":userid,
        "exch":syminfo.exchange,
        "token":syminfo.token
    }
    const getSecurityInfo = ()=>{
        const request = `jData=${JSON.stringify(orderData)}&jKey=${usession}`;
        const response = callApi("GetSecurityInfo",request);
        return Promise.resolve(response);
}
    const secInfo = await getSecurityInfo();
      return {
        qty: { min: syminfo.qty.default,
                    max: secInfo.frzqty ? secInfo.frzqty * 10 : Number.MAX_VALUE,
    ...
    _recalculateAMData() {
        ...
        this._positions.forEach((position) => {
            ...
            rpnl +=(position.totalPl) || 0;
            // console.log("OOOOOOOPL::", pl,opl)
        });
        this._accountManagerData.overallPl = pl+rpnl;
...
async function checkFreezeQty(order){
    const symbolData = window._symbolInfoMap[order.symbol]
    let frzqtyCheck;
    let multipleOffrzQty;
    let orderData={
        "uid":userid,
        "exch":symbolData.exchange,
        ...
        const request = `jData=${JSON.stringify(orderData)}&jKey=${usession}`;
        const response = await callApi("GetSecurityInfo",request);
        if (response.frzqty == undefined){
            return ({"frzqtyCheck":frzqtyCheck,"frzqty":0})
        }
        else if(Number(order.qty) < Number(response.frzqty)){
            frzqtyCheck = Number(order.qty)
            return ({"frzqtyCheck":frzqtyCheck,"frzqty":Number(response.frzqty)})
        }
        else{
            const qty = Number(order.qty)
            let frzQtyFloor = Math.round(Number(response.frzqty) / symbolData.lot_size) * symbolData.lot_size
            let frzqtyDiv = qty / Number(response.frzqty)
            multipleOffrzQty = Math.floor(frzqtyDiv)
            frzqtyCheck = qty - (frzQtyFloor * multipleOffrzQty)
            let remainingQty = (Number(response.frzqty) - frzQtyFloor)
            return ({"frzqtyCheck":frzqtyCheck,"frzqty":frzQtyFloor,"multipleOffrzQty":multipleOffrzQty,"remainingQty":remainingQty})
        }
...
function formatRequiredOrder(orderData) {
    ...
    let ptype = orderData.s_prdt_ali;
    let tp;
    if(ptype == "CNC"){
        tp = "Delivery (CNC)";
    }
    else if(ptype == "NRML"){
        tp = "Carry Forward (NRML)"
    }
    else if(ptype == "MIS"){
        tp = "IntraDay (MIS)"
    }
    ...
    const order = {
        ...
        limitPrice: orderData.prc == "0.00" ? orderData.rprc ? parseFloat(orderData.rprc) : "" : parseFloat(orderData.prc),
        profit: 0,
        ...
        stopPrice: orderData.prctyp == "LMT" ? 0.00 : parseFloat(orderData.trgprc) || 0.00,
        symbol: orderData.exch+":"+orderData.tsym,
        type: priceTypes[orderData.prctyp],
        transType: orderData.trantype == "B" ? "BUY" : "SELL",
        execution: "",
        takeProfit: orderData.bpprc ? orderData.bpprc : 0,
        stopLoss: orderData.blprc ? orderData.blprc : 0,
        ...
        ordType: orderData.prctyp,
        product:orderData.s_prdt_ali,
        ...
        customFields:{'productType':tp},
        orderParams: orderData,
        borelatedorderid:orderData.snonum ? orderData.snonum : ""
    };
   return order;
}
...
function formatRequiredPosition(positionData) {
    ...
    const order = {
        ...
        side: isBuy ? 1 : -1,
        avgPrice: Number(positionData.upldprc) > 0 ? Number(positionData.upldprc) : Number(positionData.netavgprc),
        ...
        overallPl: 0,
        netavgprc: Number(positionData.upldprc) > 0 ? Number(positionData.upldprc) : Number(positionData.netavgprc),
        bAvg:bavg,
        sAvg: savg,
